...
class Parser:
    ...
    def dot_after_nonterminal(self, production):
        dot_index = production.rhs.index(".")
        if dot_index == 0:
            return False, None
        if production.rhs[dot_index - 1] in self.grammar.getNonterminals():
    ...
    def move_dot(self, production):
        dot_index = production.rhs.index(".")
        return Production(production.lhs,
                          list(production.rhs[:dot_index]) + list(production.rhs[dot_index + 1]) + list(["."]) + list(
                              production.rhs[dot_index + 2:]))
